...
contract('MasterChef', ([alice, bob, carol, referrer, treasury, dev, fee, owner]) => {
    beforeEach(async () => {
        ...
        this.referral = await PaiReferral.new({ from: owner });
        this.chef = await MasterChef.new(this.pai.address, '100', '1000', { from: owner });

        await this.pai.transferOwnership(this.chef.address, { from: owner });
        await this.referral.updateOperator(this.chef.address, true, { from: owner });
        await this.chef.setPaiReferral(this.referral.address, { from: owner });
